...
export default function PrerequisiteForm() {
  ...
  const [DisplayPrereqErrorMessage, setDisplayPrereqErrorMessage] =
    useState("");
  const [DisplayPrereqSuccessMessage, setDisplayPrereqSuccessMessage] =
    useState("");

  const [checkBoxChangesErrorMessage, setCheckBoxChangesErrorMessage] =
    useState("");
  const [checkBoxChangesSuccessMessage, setCheckBoxChangesSuccessMessage] =
    useState("");
  ...
  const [
    prereqUpdateAvailResultSuccessMessage,
    setPrereqUpdateAvailResultSuccessMessage,
  ] = useState("");
  const [
    prereqUpdateAvailResultErrorMessage,
    setPrereqUpdateAvailResultErrorMessage,
  ] = useState("");
  const [prereqUpdateAvailResultVal, setPrereqUpdateAvailResultVal] =
    useState(false);
  ...
  const handleDisplayPrerequisites = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://sallg001-cs418-course-project.onrender.com/prerequisites"
      );
      if (!response.ok) {
  ...
  const onChangeCheckBoxEvent = async (e, index, id, prereq) => {
    ...
    console.log(
      "Current checked state = " + res[index].enable_disable + "\n\n"
    );
  ...
  const handleCheckBoxChanges = async (fixedToggleVal, id) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://sallg001-cs418-course-project.onrender.com/prerequisites/togglePrerequisites",
        {
