...
course.get("/:id", (req, res) => {
  connection.execute(
    "SELECT a.course_id, a.course_tag, a.credit_hours, b.prereq_set_num, GROUP_CONCAT(c.prereq_id) as prereq_ids, GROUP_CONCAT(d.prereq_tag) as prereq_tags, GROUP_CONCAT(d.credit_hours) AS prereq_credit_hours FROM course AS a INNER JOIN course_prereqs AS b ON a.course_id INNER JOIN prerequisite_sets AS c ON b.prereq_set_num INNER JOIN prerequisites AS d ON c.prereq_id WHERE a.course_id=? AND a.course_id = b.course_id AND b.prereq_set_num=c.prereq_set_num AND c.prereq_id = d.prereq_id GROUP BY c.prereq_set_num",
    [req.params.id],
    function (err, result) {
      if (err) { 
        res.json(err.message);
